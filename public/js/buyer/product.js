$(document).ready(function () {
    ...
    if (Cookies.get("email") !== undefined) {
        getWishlistProduct(wishlist, productFeatured);
    }else{
        const wishlistItem = []
        getProduct(productFeatured, wishlistItem)
    }


    function getWishlistProduct(wishlist, productFeatured) { 
        prescoExecutePOST("api/WishListController/getWishList", wishlist, function (res) {
            let wishlistItem = [];
            
            if (res.response !== undefined) {
                res.response.forEach(wishlist => {
                    wishlistItem.push(wishlist.product_id);
                });
            }
                        ...
                        <div class="card mx-3 shadow-lg">
                            <div class="product-image">
                                ...
                                <div class="wishlist-container d-flex flex-column align-items-center justify-content-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" fill="currentColor" class="bi bi-heart-fill ${wishlistItem.includes(product.product_id) ? 'added-to-wishlist' : ''}" viewBox="0 0 16 16">
